...
- **What should L18 own?** **`comment` in its manner/state sense — one identity, `adverb-comment`, in two already-owned hosts** (`C'est comment ?` = L3's `chunk-c-est` + `comment`, in the spot L8's frozen question occupies; `Comment ça va ?` = `comment` + L17's frozen chunk). **Corrected 2026-08-08: the hosts are asymmetric** — Host A may be answered from L17's owned set; **Host B has no owned French answer and L18 requires none** (compact spec §8). **1 demand · 0 new question mechanisms · 0 new grammar systems · 0 new architecture verbs.**
