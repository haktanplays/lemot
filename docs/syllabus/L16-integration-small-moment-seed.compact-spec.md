...
> - `On y va ?`
>
> All three lines are carried by identities that already exist (`chunk-il-faut` + `chunk-faire-une-pause` · `chunk-je-vais` + `chunk-a-la-maison` · `chunk-on-y-va`). **No alternative readings are generated from this decision** — this is the reading.
>
> **Unchanged by this decision:** L16 stays `journeyRole: "integration"` with **zero** acquisition demands (`acquisitionDemandItemIds: []`), takes **no** budget exception, and introduces **no** new grammar system, architecture verb, or lexical item. The PRJ-015 ruling that L16 contributes **0** active-new is not reopened.
>
> **Forward hook, retired and re-locked:** the fatigué-carried L17 feelings preview is **retired** (its carrier does not exist) and is **not** replaced by an L18 futur-proche preview or by any new L17 identity. In its place:
>
...
| **Why a compact spec is sufficient** | L16 introduces **0 new grammar systems / 0 new architecture verbs / ≈0 new lexis** — it recombines L11–L15 and adds **one** bounded meta beat (the A Small Moment seed). The full template's heavy item tables and sound-pattern section would be padding (same call L10/L13 made). A compact spec captures the recombined target, the strict seed definition, the deferred set, the scene, and the AI guardrail. |
...
> Scene (after-class human context, continuous with L10/L13/L15): *"After class. A break is needed before anyone goes anywhere; then home — and someone asks if you're coming."* The A Small Moment read **is** the scene; the spine is **read → understand → respond simply**.

### 5.1 Micro-reading candidate evaluation (history + the canonical outcome)
...
- `On y va ?` *(L14 — frozen `on y va`, rising intonation; the place is already named on line 2, so `y` is not stranded)*

> **Two demotions, one lesson each.** (1) The briefed variant's `Après,` was a **new-vocabulary leak** — the classic A Small Moment risk of a "richer" reading sneaking in a connector. **If a connector is ever wanted, it needs its own lesson; not here.** (2) B′'s `Je suis fatigué.` was a **phantom-ownership leak** — a line assumed owned because it *reads* like L2/L6 material, while no identity carries it. Prose "owned" claims are not evidence; only a registry identity is.
...
| (small-moment reading) | `sent:l16-small-moment-reading-1/-2/-3` | supported (composed) | the ≤2–3 line read (`Il faut faire une pause.` / `Je vais à la maison.` / `On y va ?`) — **present-only, known-items-only** |
...
| forward hook | *(none — retired)* | **n/a** | The fatigué-carried `phen:feelings-preview` is **retired**: no `fatigué` identity exists and none will be created. It is **not** replaced by `phen:futur-proche-preview` (L18) or by any new L17 identity. **L16's small-moment experience is itself the semantic / human-context preparation for L17** — a lexical preview is permitted by the archetype but is not required, and L16 carries none. |
...
| **Carry-out** | → **L17 human context / feelings** — carried **semantically**, by the human moment L16 lets the learner read and answer, **not** by a lexical preview · → **L19 integration / repair + a light *second* A Small Moment recurrence** · → **paid-zone deeper A Small Moment / live AI feedback / personalization** · → later **Practice Pool Challenge** (open production, paid-deepened) |
...
> **Principle check** (engine §8): introduces new (the *A Small Moment reading-response format*) ✓ · grows old (recombines **L11–L15** — the richest chain yet) ✓ · prepares future (A Small Moment recurs L19; L17 is prepared **semantically**, by the human moment, not by a previewed word) ✓.
...
|---|---|---|---|
| **Meet It** | Meet the after-class pause/home moment | listen `Il faut faire une pause. Je vais à la maison. On y va ?` (TTS, FR only) | passive mirror |
...
| **Weave It** | Build a chained after-class line | assemble `Il faut faire une pause, je vais à la maison.` / `Est-ce que je peux faire une pause ?` | model + alternatives |
...
| **Say It Your Way** | Run the after-class moment yourself | "Class is over — say one must take a break, ask if you can, and say where you're going." | model-answer-only (no AI) + natural alternatives |
| **Natural Reveal** | Why it's natural; obligation ≠ advice; A Small Moment ≠ chat | read reveal | natural upgrade + register note |
...
|---|---|---|---|---|
| **A Small Moment** *(pattern, not a word)* | `phen:a-small-moment-seed` | "a tiny situation you can read, understand, and answer — not a chat bot" | "Il faut faire une pause. Je vais à la maison. On y va ?" | new (pattern) |
...
*(The former "(recognition) feelings / futur preview" row is removed: its carrier `fatigué` has no identity, and the founder decision neither creates one nor substitutes an L18 futur preview. L16 surfaces no forward-preview entry in Mon Lexique.)*
