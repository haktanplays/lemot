...
>
> **Answer inventory is narrow by design.** The only owned answers are **`Ça va.` · `Ça ne va pas.` · `Je suis fatigué(e).` · `Je suis content(e).`** (all L17). The registry contains **no descriptive adjectives** — AI must **not** manufacture any (`C'est bon.` / `C'est super.` / `C'est joli.` are blocked) to make `C'est comment ?` feel richer.
>
