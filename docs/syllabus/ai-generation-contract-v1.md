...
| **L22** | Bounded PRICE question only: `C'est combien ?` *(the default)* · `Un café, c'est combien ?` · `Le café, c'est combien ?` · plus the **recycled** order it precedes, `Je voudrais un café, s'il vous plaît.` · price answers written **only** as a numeral plus the euro symbol (`2 €`, `3 €`) | **`combien de + noun`** in any form — it needs `de`, plural noun morphology and determiner deletion, none of which is owned · `de` · `des` · plural morphology · **`en`** · partitives · `ça fait combien ?` · **`combien ça coûte ?` / `coûter`** *(no verb exists and `C'est combien ?` is more idiomatic anyway)* · `vous en voulez combien ?` · `combien êtes-vous ?` · **any French number word** (`deux`, `trois`, …) · **`euro` / `euros` as French text** · counting sequences · 1–10 lists · time · age · phone numbers · arithmetic · `l'addition` · a second new question word · `qu'est-ce que` / inversion / embedded questions · new nouns · `C'est bon.` used about a **price** rather than the coffee's quality · **any copy implying `chunk-c-est` is newly owned** · **any claim the learner can understand a SPOKEN price** *(the product has no listening exercise)* · `Je ne comprends pas.` / `Vous pouvez répéter ?` / `C'est pas grave.` *(RR-A)* · past tense · futur proche · any unlock / score / threshold / Campfire-arrival copy | L22 = **Price**. **journeyRole standard ; active-new exactly 1 (`adverb-combien`) ; known-items-only otherwise.** **L22 opens ONE price question. It does NOT open quantity grammar and it does NOT open the number system.** The sequence's broad job (*how much, how many*) was resolved at the implementation gate to **price-light**: productive quantity needs three reserved systems, so `combien` is owned first through price and quantity use stays a **later doorway**. The ownership unit is the **word**, as in L18 — no host frame becomes an identity, so `chunk-c-est-combien` does not exist. **`chunk-c-est` is NOT promoted.** `combien` occupies the same end slot the learner has worked since L8 (`où`) and L18 (`comment`); word-hood is proved by that slot control plus the **bare / named alternation**, NOT by L18's two-owned-hosts test, which French does not make available here. **PRICE ANSWERS ARE NUMERALS, NOT FRENCH**: `2 €` is shared orthography, so nothing is claimed about `deux` or `euros` and neither word may appear — and the numeral must **never** sit on a `tts: true` surface or in a meet-card `fr`, because French TTS would read it aloud as *deux euros* and smuggle in an undeclared form. **The arc is ask the price → decide → order**, and the owned order must appear: it is why the question exists |
| **L23** | **Owned-language recombination only** — nothing new. The counter: `Bonjour !` · `Un café, c'est combien ?` / `C'est combien ?` / `Le café, c'est combien ?` · a price shown as a **numeral + `€`** · then EITHER `Je voudrais un café, s'il vous plaît.` + `Merci.` OR `Merci, au revoir.` Afterwards, to someone you know: `Le café, c'est comment ?` *(they ask)* → `C'est bon.` / `Ce n'est pas bon.` | **any acquisition at all** *(L23 is `[]`)* · **`Non, merci.`** — `chunk-non-merci` is registry-**supported**, never a demand; the owned decline is `Merci, au revoir.` · **price adjectives** `cher` / `pas cher` / `trop cher` *(none exists)* · `mauvais` · `j'aime` / `j'adore` · `mais` · any French number word · `euro` / `euros` · `combien de` · quantity `de` / `des` · `en` · partitives · `coûter` · `ça fait combien` · `l'addition` · any new noun, adjective, verb or connective · **a social-state opening** (`Comment ça va ?` + a state answer) — that is L19's lesson and its presence means L23 has drifted · **forced evaluation**: `C'est bon.` at the counter before the coffee has been tasted, or used about a **price** · `Je ne comprends pas.` / `Vous pouvez répéter ?` / `C'est pas grave.` *(RR-A)* · past tense · futur proche · a second new question word · any copy implying `chunk-c-est` is newly owned or that the learner understood a **spoken** price · **any threshold / arrival / Campfire / journey-complete claim** *(that is L24's job)* · any unlock / score / percentage / badge copy | L23 = **Transaction Integration**. **journeyRole integration ; acquisition exactly 0 ; known-items-only.** **L23 integrates a transaction. It does NOT teach transaction vocabulary.** Generate from the **LessonContract + a scratch `surface-inventory.md`** — ownership → sentence families → screens, the L22 authoring sequence, and the inventory must exist before any screen is written. **The architecture is MODEL T, two beats welded by consequence:** the counter, where the **price determines the ending**, and afterwards, where the learner **reports** what it was like — possible only because they bought it. **The distinguishing requirement, which no shipped lesson makes: an external fact the learner obtained by asking decides what they say next.** A majority of production beats must be **DEPENDENT** on the beat before; mostly-independent drills mean the architecture is wrong. **Beware the two near-misses:** L22's exit *already* runs the transaction end to end, so L23 is not "L22's say-it plus one sentence"; and a social opening plus a state answer is L19, not this. **Agency lives in beat 2** (good or not is the learner's truth) — the contingent branch is dependency, not choice. **No coverage sweep**: `ça va`, `je suis`, `il faut`, `on y va`, `est-ce que` are available and deliberately not required. Support opens **no higher than `mid`**, never `supported`. The price display keeps L22's boundary exactly: numeral + `€`, no `itemId`, no chip, no evidence target, never on a `tts: true` surface, never said |
...
>
> **L22 note (one price question, not a numbers curriculum).** L22 opens **`combien`** and nothing else. The sequence decision gave it the broad job *quantity / price*; the implementation gate resolved that to **price only**, because every natural "how many" — `Combien de cafés ?`, `Vous en voulez combien ?` — requires `de`, plural noun morphology, determiner deletion or `en`, all of which the pre-Campfire band reserves. AI must generate the **price question inside the owned `c'est ___` slot** and the **owned order that follows it**, and nothing more.
>
> **Three failure modes are specific to a price lesson.** First, **number leak**: writing `deux euros`, or putting the numeral `2 €` on a spoken surface where French TTS reads it aloud — the price is written, never said. Second, **quantity leak**: `combien de` looks like a small step from `combien` and is three grammar systems away. Third, **currency creep**: `euro`/`euros` written out "just once" — no currency identity exists, and `€` is a symbol, not a French word.
>
> **What must NOT be inferred.** Owning `combien` does not open counting, quantity, time, age or arithmetic; it does not make `ça fait combien ?` or `combien ça coûte ?` available; and it says nothing new about `c'est`. The learner can ask what something costs. That is the whole capability, and it is enough — it turns L0's flagship order into a decision.
>
> **L23 note (integrate a transaction; do not teach one).** L23 introduces **0 systems, 0 words, 0 identities**. Its whole job is to make the learner **use** what L21 and L22 opened, inside one arc where the moves depend on each other. AI must generate **recombination of genuinely owned material only** and must not add anything to make the integration feel bigger.
>
> **Four failure modes are specific to this lesson.** First, **"L19 at a café"**: opening with `Comment ça va ?` and a state answer reproduces a shipped lesson and abandons the transaction. Second, **"L22 plus one sentence"**: L22's exit already runs greet → ask the price → decide → order, so simply appending `C'est bon.` is not integration — the difference must be the **condition** and the **report**. Third, **forced evaluation**: `bon` at the counter before tasting is semantically false, which is exactly why the architecture is two beats. Fourth, **transcript memorization**: one long required script taught line by line is not integration, and earlier screens must never pre-print the final route.
>
> **What must NOT be inferred.** Zero acquisition is not a licence for supported-new language: if a scene needs a word the learner does not own — a price adjective, a decline phrase, a number — **the scene changes, never the demand count**. And L23 practises; it does not mark a threshold. No arrival, completion or Campfire claim belongs here.
>
