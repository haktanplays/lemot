...
- **Homograph disambiguation (sense suffix).** When ASCII/accent normalization **collides two different meanings** into the same slug, add a short, semantic, **stable sense suffix**. Examples: `où` (where) → `word:ou-where`, `ou` (or) → `word:ou-or`; `là` (there) → `word:la-there`, the article `la` → `word:la-article` (or its existing article ID if already defined); **`y` (place pronoun) → `word:y-place`** (reserve `word:y-*` for other senses — keep existential **`il y a` as a frozen `chunk:il-y-a`**, and `j'y pense` for a later sense; `j'y vais` → `chunk:j-y-vais`, `on y va` → `chunk:on-y-va`); **`devoir`** = obligation/must in L15 (`chunk:je-dois`) — if the **owe** sense is later opened, use `word:devoir-owe` vs `word:devoir-obligation`; **`il faut`** = `chunk:il-faut` (the high-frequency surface of the lexeme `word:falloir`; `faudrait`/`fallu` are later forms, **not** owned by L15); **`ça va`** (L17) = the **frozen social check-in** `chunk:ca-va` / `chunk:ca-va-question` / `chunk:ca-ne-va-pas` — the `va` here is **not** an `aller`/futur-proche ownership signal; future `aller`/futur uses of `va` stay **separate** and must be status-marked by later lessons (cf. `il y a` vs place-`y`). This is a **going-forward** convention — older specs are **not** retroactively rewritten in this step (§11). **Learner-facing labels keep correct French spelling** (`où`, `ou`, `là`, `la`); the disambiguation lives **only** in the internal ID.
