...
> **A legal canonical identity is not automatically an acquisition unit.** `[FL — founder-ratified 2026-08-06]` This section answers only *"may this form exist as a canonical item?"* — **identity is cheap; existing costs nothing.** Whether that identity consumes an **active-new slot** is a separate question, decided by acquisition accounting (§6.9.1 → PRJ-015), and whether it may be a **UI chip** is a third question (§4.4/§4.6). **Chip eligibility and acquisition accounting are independent decisions and must not be inferred from one another.**

**4.4 No full-sentence chips.** `[ADR-0004]` (CB-03) No full-sentence or multi-clause chips as primary UI chips; a sentence may be a model answer, never a UI pill. *(CI-lint enforcement = Engineering.)* **This is a *rendering* rule.** It governs what may appear as a pill; it says nothing about whether a form may exist (§4.3) or whether it consumes an active-new slot (§6.9.1).
...
> **These classes are a *rendering exception*, not an accounting whitelist.** `[FL — founder-ratified 2026-08-06]` They lift §4.4's UI-pill ban for specific sentence-shaped forms. **They must not be read as the only forms permitted to consume an active-new slot.** Their acquisition accounting follows PRJ-015 like any other identity: where independently owned, `je ne suis pas` and `ce n'est pas` each count as **one acquisition demand** — **protected status does not itself create the slot; independent ownership does** (§6.9.1).
...
**6.7b Two axes: content archetype and journey role.** `[FL — founder-ratified 2026-08-05]` (CB-18) A lesson is described by **two independent taxonomies**. They are **not synonyms**, no one-to-one map exists between them, and **neither can replace the other**. The historical use of the single word *"archetype"* for both is the source of a long-standing routing conflict.

| Axis | Canonical term | Current / future representation | Owns |
|---|---|---|---|
| Linguistic / instructional engine | **content archetype** | current `primaryArchetype` / `secondaryArchetype` | section weighting · content-specific item budgets · content-specific **production planning targets** · French-QA and authoring risks |
| Pedagogical job in the journey | **journey role** | **`[OPEN — future `journeyRole` metadata field]`** | **active-new contracts** · Standard-only authoring defaults · Integration / Review exceptions |

> **Evidence for the separation.** One content archetype spans several journey roles and vice-versa: lessons declaring `architecture-verb` describe themselves in their own prose as both normal engine teaching *and* as narrow doorways; lessons declaring `chunk-natural-speech` span entry, ordinary teaching, and integration work. The journey role is therefore not derivable from the content archetype.

> `[OPEN]` **Legacy mixed-axis values.** The current content-archetype union contains function-like values — notably `review-integration` and `summit-milestone` — which describe a journey role rather than a linguistic engine. They are **legacy conflations awaiting a future metadata reconciliation**. They are **not** deprecated, renamed, or migrated by this decision.

> **Content-archetype definitions, weighting, budgets, and production planning targets are owned by [`docs/syllabus/lesson-archetype-templates-v1.md`](../../syllabus/lesson-archetype-templates-v1.md)** — not by this Bible and not by the Lesson Flow Canon. This Bible owns the *existence and distinction* of the two axes, and the journey-role contracts below.

**6.8 Journey-role contracts & load formula.** `[DR]` (Q2, CB-17/18; axis clarified 2026-08-05) Every lesson names a primary **content archetype** (+≤1 secondary); ~70% template / 20% archetype-weighted / 10% bespoke (bespoke needs justification). Separately, the **journey role** carries the novelty contract. Per-**journey-role** activeNew budgets, unchanged: **Doorway 1–2 · Standard 1–4 · Integration 0 · Review 0 · Milestone 0–3**. `totalProductionLoad = activeNew + supportedTarget + productionCarryover + repairItem + integrationTarget`; a lesson cannot dodge the activeNew cap by hiding old load in other buckets. *Labeled derived, founder-ratified; not a runtime-enforcement claim.*

> **Doorway / Standard / Integration / Review / Milestone are journey roles, not content archetypes.** They must not be written into `primaryArchetype`, and `primaryArchetype` must not be read as a journey role.
...
> **Composition is the other half of the same principle.** Just as *status* does not establish ownership, neither does *having an ID*. Where a composite identity's responsibility is already carried by separately owned components, it consumes no additional slot — see §6.9.1 and PRJ-015 IC-003/IC-004.
...
**6.8.6 Chip-composition accounting — resolved 2026-08-06.** `[FL]` The apparent conflict between §4.3's composition classes, §4.6's frozen classes, and §6.9.1's counting was a **routing/interpretation problem, not a policy contradiction**: the four questions sit at different levels (content identity §4.3 · rendering §4.4 · rendering exception §4.6 · **acquisition accounting** §6.9.1 → PRJ-015). Counting semantics now route explicitly to **PRJ-015 IC-001/IC-003/IC-004** (§6.9.1). **Journey-role assignments in §6.8.5 are unchanged**, and the L6 exception (§6.8.3) **remains necessary**. Remaining work is Engineering, not policy: `acquisitionLink` coverage (§20.2).
...
| Budget | Standard journey role | Status | Axis owner | What it counts |
|---|---|---|---|---|
| **Active-new load** | **1–4** | founder-ratified | **journey role** (§6.8) | distinct newly owned producible chips/items |
| **Meaningful production actions** | **no universal floor** | **global target retired 2026-08-05** | **content archetype** (templates doc) | separate occasions the learner constructs an intended French response |
| **Authored French surfaces** | **~30–35** | authoring inventory target | journey role (Standard default) | reviewable learner-facing utterances prepared across the lesson's **full content inventory** |

> **The third row is an authoring-inventory figure, not a rendered-path figure.** A single learner path renders a pedagogically selected **subset** of the authored inventory. Rendered-encounter count is a **diagnostic**, not a ratified numeric invariant — see §6.9.3. Content-archetype templates may shape the *composition* of that inventory; the ~30–35 figure itself is a Standard journey-role authoring default.

**6.9.1 Active-new load `[FL]`.** Counts **distinct new chips/items whose independent production is expected**, once per distinct item however many times the learner uses it. It does **not** count: complete sentences as separate active-new items · supported-new pieces · recycled items · recognition-only material · ghost/exposure material · interlocutor language · model-only reveals · repeated attempts using the same new item.

> **Counting semantics route to PRJ-015.** `[FL — founder-ratified 2026-08-06]` **Active-new accounting follows the `PRJ-015 Item-Counting Contract v1.0` — IC-001 / IC-003 / IC-004.** Canonical identities **do not multiply acquisition load merely by existing**. Count the **primary independently owned acquisition responsibility**, not every representation. This Bible continues to own the **active-new 1–4 invariant itself** (PRJ-015 §1 boundary); PRJ-015 owns the **counting semantics** and is **not duplicated here**.
>
> - **IC-001** — three counting contexts: *identity* (what units exist and which are linked) · **acquisition** (the new demand a lesson places on the learner — **this is the context of active-new 1–4**) · *presentation/load*.
> - **IC-003** — when distinct linked identities **jointly open one productive learner capability**, acquisition counts them **once**.
> - **IC-004** — each separately owned unit or sense has **one primary acquisition identity**; **linked sub-identities never multiply the acquisition count by existing**.
>
> **`identity count ≠ acquisition count`.**

**6.9.1a Independent ownership determines the slot.** `[FL — founder-ratified 2026-08-06]` A canonical item consumes **one** active-new slot when the lesson assigns it **independent acquisition / production responsibility**. It does **not** consume an additional slot merely because it has its own ID · is registry-active · is sentence-shaped · is idiomatic · is a useful model identity · or represents a composite result. **Where separately owned components already carry the acquisition responsibility, the composite identity does not multiply the count.**

Carried forward unchanged from §6.8.4: **registry status alone is insufficient**, **lesson-level responsibility governs ownership**, and **first encounter ≠ supported use ≠ active ownership**.

> **The complete-sentence exclusion above is an anti-double-counting rule.** It prevents a whole and its components being counted *separately* for one instructional responsibility. It is **not** a sentence-shape ban on ownership · **not** a `PROTECTED_CHUNKS` whitelist · and **not** a claim that all composites are free. A frozen or idiomatic whole whose components are **not** separately owned **is** the ownership unit and counts once.

> **Formula chunks are neither globally exempt nor automatically counted** — they follow the same ownership-unit rule. Audit-supported illustrations: `au revoir` is **independently owned → one demand**; `non merci` is **Supported, not independently owned → no demand**. *(The ratified L6 Integration exception at §6.8.3 is unaffected and still required.)*

> **Composite / accounting identities — audit-supported examples, not registry state.** `j'ai faim` · `j'ai une question` · `j'y vais` · `je suis ici` may exist as canonical identities **without multiplying acquisition count**, because their responsibility is carried by component ownership units. **No registry metadata has been corrected**; see §20.2 for the Engineering follow-up. **A narrow active-new budget is not a narrow production surface** — the ceiling bounds *new ownership*, not *how often the learner works*. The §6.8 anti-gaming rule stands unchanged: a lesson cannot hide genuinely new production load inside supported, carryover, repair, integration, or any other role merely to stay under the active-new ceiling.

**6.9.2 Meaningful production action `[FL]`.** **No universal production-action floor is ratified.** `[FL — 2026-08-05]` The provisional global **5–8** target is **retired before ratification**: three audited lessons produced 5, 4, and 3, the single number flattened genuine content-archetype variation, and a raw count cannot detect the structural weaknesses that actually matter (a lesson with no unscaffolded retrieval, or an exit proof that duplicates an earlier guided target, can still hit any number). The earlier global `3–5` band is **not** restored, `4–8` is **not** adopted, and **no replacement universal range is set**.

> **Numeric production planning belongs to the content-archetype axis.** Per-archetype authoring targets are defined in [`docs/syllabus/lesson-archetype-templates-v1.md`](../../syllabus/lesson-archetype-templates-v1.md) and are **not** reproduced here. They are **content-archetype planning targets** — not a single global invariant, not runtime enforcement, and **not proof of lesson quality by themselves**.

> **Cross-archetype production quality will be governed by structural requirements rather than one universal numeric floor.** `[FL — direction ratified 2026-08-05]` `[OPEN]` The structural contract itself is **not specified or ratified here**. Candidate dimensions for a later specification — **not current invariants** — include: retrieval without full answer support · meaningful changed-dimension reuse · recombination or a genuine exit proof · avoidance of duplicate guided/open targets · active-new responsibility staying inside the applicable journey-role contract (§6.8).

**The definition below is unchanged and remains live.** A meaningful production action counts when the learner must **retrieve, choose, assemble, complete, type, or say an intended French response**. A repeated use of the same material counts again **only when at least one meaningful dimension changes**: operation · context · communicative purpose · slot value · contrast · recombination · support level · retrieval demand. Distinct production purposes may include scaffolded retrieval, reconstruction, slot/context variation, contrast or repair, recombination, and a short open or semi-open exit proof. **Does not count:** passive reading · tapping Continue · replaying audio · viewing a Natural Reveal · interlocutor/input-only French · recognition-only choice where no French response is constructed · the same answer rendered twice on one screen · punctuation-only or formatting-only variation · repeated identical submission with no changed demand · model-answer-only content. **A lesson must not manufacture production actions by splitting one cognitive act into artificial taps** — this holds under any archetype target, and is why no universal floor is set.
...
**6.9.5 Journey-role scope `[OPEN]`.** The **~30–35** authored-surface target is stated for the **Standard journey role** only. **Doorway, Integration, Review, and Milestone keep their §6.8 activeNew contracts unchanged**, and **no authored-surface or rendered-density band is invented for them here**. Production planning is not scoped here at all — it belongs to the content-archetype templates (§6.9.2). Journey-role scoping is **unresolved and routed to founder review** (§20.2), and remains unaddressable until the `journeyRole` metadata field exists.
...
| **Global production-action floor** | **none** | **retired 2026-08-05** — no universal numeric floor is ratified | none | §6.9.2 |
| Production planning targets, per **content archetype** | see templates doc | authoring reference — **not** a global invariant, **not** runtime enforcement | none | [`lesson-archetype-templates-v1.md`](../../syllabus/lesson-archetype-templates-v1.md) |
| **Structural production-quality contract** | **`[OPEN]`** | future enforcement direction; candidate dimensions only | none | §6.9.2 |
...
**Three density budgets are distinct** (§6.9), and each belongs to a different owner: **active-new load** (1–4 Standard, **journey-role** axis — founder-ratified), **meaningful production actions** (definition live, **no global numeric floor**; numeric planning owned by the **content-archetype** templates), and **authored French surfaces** (~30–35, Standard journey-role authoring default). They count different things and must never be summed, equated, or traded against each other.
...
2b. Standard journey role: ~30–35 **authored** French surfaces across the full inventory? Production actions counted per the §6.9.2 definition and planned against this lesson's **content-archetype** target (templates doc) — no artificial taps, no passive reveals or extraction duplicates counted, no added screens or minutes to hit a number, and no assumption that every authored surface renders? (§6.9)
...
- **`acquisitionLink` registry extension** `[OPEN — Engineering]`: the existing `acquisitionLink { role: "primary" | "linked" }` relation (already used for `chunk-un-the` ↔ `noun-the`) should later be extended to the composite/accounting identities that currently lack explicit links. Audit-supported candidates: `chunk-j-ai-faim` · `chunk-j-ai-une-question` · `chunk-j-y-vais` · `chunk-je-suis-ici`, expected direction `role: "linked"` with the appropriate primary ownership identity. **Do not author incorrect link structures merely to fill the field**; where the current schema cannot cleanly represent **multiple** primary ownership components, flag it for the Engineering pass. **No `countsAsActiveNew` (or any duplicate boolean) may be introduced** — the active-new result must remain **derived** from acquisition-identity links, lesson-level production responsibility, and support state. PRJ-015 §5 already anticipates this as a *"separate Engineering task"*. **`acquisitionLink` coverage is currently incomplete.**
- **Automation principle (recorded, not implemented)** `[OPEN]`: *deterministic inputs* — `acquisitionLink`/ownership identity · lesson-level production targets · suggested/required pieces · `supportRole` · evidence responsibility · prior-lesson ownership. *Human authoring decision* — semantic classification where needed (idiomaticity, frozen status, correct acquisition identity). **Once ownership metadata is authored, counting should be deterministic.** No runtime algorithm is specified.
...
- **2026-08-06 (second pass) — Active-new counting routed to acquisition identity (§4.3, §4.4, §4.6, §6.8.4, §6.8.6, §6.9.1, §6.9.1a, §20.2).** `[FL]` The apparent conflict between §4.3 composition classes, §4.6 `PROTECTED_CHUNKS`, and §6.9.1 counting was a **routing/interpretation problem, not a policy contradiction** — four questions at four levels: **content identity** (§4.3) · **rendering** (§4.4) · **rendering exception** (§4.6) · **acquisition accounting** (§6.9.1 → PRJ-015). **Active-new accounting now routes explicitly to `PRJ-015 Item-Counting Contract v1.0` IC-001/IC-003/IC-004**, whose ownership-unit model is ratified: **`identity count ≠ acquisition count`**, and canonical identities **do not multiply acquisition load merely by existing**. **Independent ownership determines the slot** (§6.9.1a) — not having an ID, being registry-active, being sentence-shaped, being idiomatic, or representing a composite result. The complete-sentence exclusion is clarified as an **anti-double-counting rule**, *not* a sentence-shape ban, *not* a `PROTECTED_CHUNKS` whitelist, *not* a claim that composites are free. `PROTECTED_CHUNKS`/`SURVIVAL_FORMULAS` are a **rendering exception**; `je ne suis pas` and `ce n'est pas` count one each **because they are independently owned**, not because they are protected. **Formula chunks are neither globally exempt nor automatically counted** (`au revoir` = 1; `non merci` = 0). Composite identities `j'ai faim`, `j'ai une question`, `j'y vais`, `je suis ici` may exist **without multiplying the count** — audit-supported examples, **not** a claim that registry metadata has been corrected. **Journey roles (§6.8.5) are unchanged and the L6 Integration exception (§6.8.3) remains necessary.** PRJ-015 required **no edit** — it already contained the authoritative rule and already names Content as owner of the 1–4 invariant. Recorded as Engineering follow-up: extending `acquisitionLink` coverage; **no `countsAsActiveNew` boolean**. No registry, lesson, schema, test, or runtime change.
