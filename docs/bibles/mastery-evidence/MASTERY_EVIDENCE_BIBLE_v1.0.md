...
`PROPOSED FOR FOUNDER RATIFICATION` — adopted as Canonical semantic policy through the
founder-authorized promotion of 2026-07-27 (an application of the adopted I-17 gate rule, §33, to
weakness; not a separate Matrix row) — weakness is a **claim about difficulty**, not a tally of wrong
...
longer the live semantic rule**. **Every other live polarity-family source was reconciled in the same
patch** — the complete inventory is enumerated path by path in the Source Conflict and Gap Map **§20.1**,
which is the single owner of that list; this Bible deliberately does **not** restate a count, so the two
documents cannot drift apart. **No code, tag, threshold or runtime changed** (§41).
