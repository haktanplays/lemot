...
> **This register decides nothing.** A row marked `PROPOSED` is this Draft's proposal, not a rule.
> A row marked `CURRENT-REALITY` describes code and is **not** a decision — promoting the Bible does
> not canonize it.
...
| ME-004 | Does an event automatically constitute admissible evidence? | No. Admissibility is a separate gate. | `PROPOSED` | `PROPOSED` | Mastery & Evidence | Yes (implied by FQ-6/FQ-8) | Before promotion |
...
| ME-007 | What creates weakness? | Only verified learner-sourced error. | `INH-CANONICAL` | `LOCKED` | Mastery & Evidence | No | — |
| ME-008 | What are the error-source classes? | learner · content · validator · UI-flow · tone · AI-generator · mastery-mapping. | `INH-CANONICAL` | `LOCKED` | Mastery & Evidence | No | — |
...
| ME-011 | How many result buckets exist? | Five in code (success · precision · spelling near-miss · skip · failure); four in ADR-0021. | `CURRENT-REALITY` | `OPEN` | Mastery & Evidence | **Yes — FQ-1** | **Required now** |
| ME-012 | Is `spelling_near_miss` precision or weakness-accruing? | Unresolved. Code says weakness-accruing; active ADR says precision. | `OPEN` | `OPEN` | Mastery & Evidence | **Yes — FQ-1** | **Required now** |
...
| ME-026 | What is the weakness threshold? | `wrongCount >= 3` OR any single weak tag `>= 3`. | `CURRENT-REALITY` | `OPEN` | Mastery & Evidence | Yes — FQ-7 (class, not value) | Before promotion |
| ME-027 | What is weakness keyed by? | Engine: `ItemId`. Legacy shipped: the correct-answer string. | `CURRENT-REALITY` | `OPEN` | Mastery & Evidence | **Yes — FQ-8** | **Required now** |
...
| ME-032 | May precision-only become conceptual weakness? | Never silently. | `INH-CANONICAL` | `LOCKED` | Mastery & Evidence | No | — |
...
| ME-047 | How is a non-learner-sourced event excluded? | Policy says it must be; no mechanism exists. | `OPEN` | `OPEN` | Mastery & Evidence | **Yes — FQ-6** | **Required now** |
| ME-048 | How is an already-recorded event invalidated? | Either refused at admission or offset by a compensating record — never a silent log mutation. | `PROPOSED` | `OPEN` | Mastery & Evidence (semantics) / Engineering (mechanism) | **Yes — FQ-6** | **Required now** |
...
| ME-004 | This Draft | Bible §6 | Content, Engineering | **Absent** — classification flows straight to counters | **None** | Structural gap | Invariant (proposed) | ME-047, ME-048, ME-049 | n/a | The single largest structural gap. |
...
| ME-007 | Canonical domain source | Error Tracking System, Policy Hardening 2026-07-18 `[HARD INVARIANT]` | Content, Engineering | **No error-source field** | Policy only | Canon-vs-code | Invariant | ME-008, ME-047 | n/a | Every non-success is attributed to the learner by default. |
| ME-008 | Canonical domain source | same, §Kaynak içe aktarımı §5 | Content, Operations | Absent | Policy only | Canon-vs-code | Invariant | ME-047 | 7 of 7 classes listed | — |
...
| ME-011 | Current reality vs active ADR | `mastery.ts:132-145,222-268` vs ADR-0021 Decision | Content | **Five** buckets | Unit-tested | **Direct contradiction** | Open | ME-012 | 5 buckets enumerated against code | ADR-0021 says four. |
| ME-012 | Current reality vs active ADR + 3 docs | `mastery.ts:141-144` (audit B7) vs ADR-0021, precision-policy §2, Mastery Model, Feedback & Scoring | Content, Curriculum | Accrues `weakTags` → can make an item weak | Unit-tested (`nearMissMasteryTiming.test.ts`) | **Direct contradiction, 4 sources** | Open | ME-011, ME-026, ME-032 | **Claim "precision is never failure" FAILS on `spelling_near_miss`** | The clearest weakest-member failure in the corpus. |
...
| ME-027 | Current reality, two systems | `mastery.ts` (ItemId) vs `useErrors.ts:34-45` (answer string) | Engineering | **Both live, in different stages** | Legacy shipped; engine sandbox-only | **Two incompatible systems** | Open | ME-057, ME-058 | Both implementations read in full | Same number 3, different key, different store, no attribution in the legacy one. |
...
| ME-032 | Canonical domain source | Error Tracking Policy Hardening | — | **Contradicted in practice by ME-012** | Policy only | Conflicts with current code | Invariant | ME-012 | See ME-012 | The invariant holds only for the 2-member precision set. |
...
| ME-047 | Gap | Policy (ME-007/008) vs absent field | Content, Engineering, Operations | **No mechanism** | **None** | Policy with no implementation | Open | ME-004, ME-048 | n/a | — |
| ME-048 | This Draft | Bible §23 | Engineering, Privacy | Append-only, no delete, no compensating type; tags frozen | **None** | — | Invariant (proposed) | ME-010, ME-042, ME-060 | n/a | Compaction complicates any retroactive change. |
...
| ME-057 | Current reality | `config/productStage.ts`; P3 checkpoint §3; Error Tracking System warning | Engineering, Product | `v1LessonEngine` true **only** in sandbox | n/a | — | Descriptive | ME-027, ME-058 | **All 3 product stages read; exactly one enables the engine** | The domain's most important fact. |
...
| `INH-CANONICAL` | 13 | ME-007, 008, 009, 018, 024, 025, 028, 029, 030, 031, 032, 038, 051 |
| `INH-FOUNDER` | 2 | ME-006, ME-054 |
| `NON-SIGNAL` | 3 | ME-005, ME-023, ME-050 |
| `PROPOSED` | 4 | ME-003, ME-004, ME-048, ME-049 |
| `CURRENT-REALITY` | 21 | ME-011, 013, 014, 016, 021, 026, 027, 034, 035, 036, 037, 039, 040, 041, 042, 043, 044, 045, 057, 059, 060 |
| `OPEN` | 6 | ME-012, ME-017, ME-020, ME-022, ME-046, ME-047 |
| `SUPERSEDED` | 1 | ME-033 |
...
| Status | Count | IDs |
|---|---|---|
| `LOCKED` | 24 | ME-001, 002, 005, 006, 007, 008, 009, 010, 015, 018, 019, 023, 024, 025, 028, 029, 030, 031, 032, 038, 050, 051, 054, 058 |
| `PROPOSED` | 3 | ME-003, ME-004, ME-049 |
| `OPEN` | 17 | ME-011, 012, 017, 020, 022, 026, 027, 033, 034, 043, 046, 047, 048, 052, 053, 055, 056 |
| `DESCRIPTIVE` | 16 | ME-013, 014, 016, 021, 035, 036, 037, 039, 040, 041, 042, 044, 045, 057, 059, 060 |
| **Total** | **60** | — |

> `LOCKED` here means **a decision exists in an upstream authority** — not that the rule is
> implemented, and not that this Bible ratified it. 24 locked rows and **zero** shipped evidence
> semantics are not in tension: see ME-057.

### Founder decisions required (13 rows)

| Timing | Count | IDs |
|---|---|---|
| **Required now** | 5 | ME-011, ME-012, ME-027, ME-047, ME-048 |
| **Before promotion** | 8 | ME-004, ME-017, ME-020, ME-022, ME-026, ME-033, ME-034, ME-046 |
...
`PLANNED` **0** · `EXPERIMENT` **0** · **Implemented as ratified semantics: 0.** No row in this matrix
records a ratified Mastery & Evidence rule that is live on the shipped surface — because the shipped
surface emits no evidence (ME-057).
