/**
 * Dev-only, read-only preview of the merged L1 executable content contract
 * fixture (content/learning-engine). This is NOT the live lesson renderer and
 * NOT an interactive exercise engine — it only displays the fixture + validator
 * output so we can judge whether the contract shape is renderer-friendly before
 * hardening the schema or authoring another lesson.
 *
...
const { items, contracts, exercises } = L1_CONTENT_FIXTURE;
...
export default function LearningEnginePreviewScreen() {
  const findings = useMemo(() => validateContent(L1_CONTENT_FIXTURE), []);
      ...
      >
        ...
        <View className="mb-4">
          <Text className="font-newsreader text-2xl text-lm-ink">
            L1 Contract Fixture Preview
          </Text>
          <Text className="mt-1 font-outfit text-sm text-lm-ink2">
            {contract.id} · {contract.title}
          </Text>
          <Text className="mt-1 font-outfit text-xs text-lm-ink3">
            Not the live lesson renderer. No input, no grading, no interaction
            engine. Reflects content/learning-engine only.
          </Text>
        </View>
        ...
        <Section
          ...
          subtitle="validateContent(L1_CONTENT_FIXTURE)"
        >
        ...
        <Section title={`Items (${Object.keys(items).length})`}>
          {Object.values(items).map((item) => (
            <View
              key={item.id}
              className="mb-3 border-b border-lm-border pb-3"
            >
