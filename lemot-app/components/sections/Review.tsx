...
import type { ReviewItem, CrossingBlank } from "@/lib/types";
 ...
 * fill-with-context, and crossing blanks. Tracks score across all items
...
export function Review({ items, onComplete, onError, say }: ReviewProps) {
  ...
  // Crossing blanks state: tracks text input and done status per blank
  ...
  /** Handle crossing blank check */
  function checkCrossingBlank(blank: CrossingBlank, blankIndex: number) {
  ...
  return (
    ...
    <ScrollView>
      ...
      >
        ...
        {/* ── Crossing blanks ── */}
        {current.type === "crossing" && "blanks" in current && (
          <CrossingReview
            item={current as { type: "crossing"; en: string; blanks: CrossingBlank[]; full: string }}
            fgInputs={fgInputs}
            fgDone={fgDone}
            onInputChange={(key, value) =>
              setFgInputs((prev) => ({ ...prev, [key]: value }))
            }
            onCheck={(blank, i) => checkCrossingBlank(blank, i)}
            onAdvance={advance}
            isLast={isLast}
          />
        )}
...
// ─────────────────────────────────────────────────────────────
// Crossing sub-component (kept in same file for cohesion)
// ─────────────────────────────────────────────────────────────

interface CrossingReviewProps {
  item: { type: "crossing"; en: string; blanks: CrossingBlank[]; full: string };
  fgInputs: Record<string, string>;
  fgDone: Record<string, boolean>;
  onInputChange: (key: string, value: string) => void;
  onCheck: (blank: CrossingBlank, index: number) => void;
  onAdvance: () => void;
  isLast: boolean;
}

function CrossingReview({
  item,
  fgInputs,
  fgDone,
  onInputChange,
