...
> [!canon] **Ders başına journey-role ataması ratify edildi (2026-08-06).** Kanonik harita **yalnızca Content Bible §6.8.5**'te yaşar; burada tekrarlanmaz. **L0** beş rolün dışındadır (bootstrap; `journeyRole` boş bırakılır, altıncı enum değeri **yok**). **Integration = 0** korunur (`0–1` değil); **L6** adı konmuş, derse özgü tek-formül istisnasıdır (`au revoir`) ve **genellenmez**. **`Hybrid` journey role yoktur** — Standard bir ders yoğun geri dönüştürebilir.

> [!warning] Bu sayılar ayrı şeyleri sayar ve toplanamaz; *Policy Hardening* bölümündeki activeNew sözleşmeleri **değişmedi** ve Doorway/Integration/Review/Milestone için yazılmış-yüzey veya render bandı **uydurulmadı** (**[OPEN]**). `journeyRole` metadata alanı **koda eklenmedi** → sözleşmeler makine ile **denetlenemez**. Chip-composition muhasebesi 2026-08-06'da **çözüldü** (sahiplik birimi = `PRJ-015` IC-001/IC-003/IC-004; kimliğe sahip olmak active-new slotu **kurmaz**) ve journey-role taksonomisini **etkilemez**; Review/Milestone'un sevkedilmiş örneği **yoktur**. Tanım evi: Content Bible v1.0 §6.7b/§6.8/§6.9. Ekran sayısı ve süre **sayı tutturmak için artırılamaz**; hiçbir sayı runtime'da enforce edilmez.
